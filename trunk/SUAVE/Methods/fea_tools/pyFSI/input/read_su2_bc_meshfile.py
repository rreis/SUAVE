...
def read_su2_bc_meshfile(mesh_filename,sol1):
    ...
    for line in file:
        ...
        if (line[0:16]=='MARKER_TAG= Wall'):
            ...
            for line in file:
                if(line_count==0):
                    element_no= [int(s) for s in line.split() if s.isdigit()]
                    sol1.no_of_elements=element_no[0]
                    sol1.elemlist = [ CTRIA3() for i in range(sol1.no_of_elements)]
                    line_count=line_count+1

                else:
                    element_no= [int(s) for s in line.split() if s.isdigit()]
                    #print element_no
                    sol1.elemlist[count].type='CTRIA3'
                    sol1.elemlist[count].eid=count+1
                    sol1.elemlist[count].g1=element_no[1]+1
                    sol1.elemlist[count].g2=element_no[2]+1
                    sol1.elemlist[count].g3=element_no[3]+1
                    

                    
                    count = count +1

                if(count==sol1.no_of_elements):
                    break
