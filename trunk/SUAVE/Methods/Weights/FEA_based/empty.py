...
#import weight_estimation_framework

# ----------------------------------------------------------------------
#  Method
# ----------------------------------------------------------------------

def empty(vehicle,primary_structure_weight):
    ...
    if not vehicle.wings.has_key('main_wing'):
        ...
        #wt_wing    = wing_main_secondary(S_gross_w,b,lambda_w,t_c_w,sweep_w,Nult,TOW,wt_zf)
...
#wt_fuselage        = tube_secondary(S_fus, diff_p_fus,w_fus,h_fus,l_fus,Nlim,wt_zf,wt_wing,wt_propulsion, wing_c_r)
...
#    aircraft_primary_structure = FEA_weight()
#    aircraft_primary_structure.import_from_suave(vehicle)
#    aircraft_primary_structure()
    wt_primary_structure       = 1.5*primary_structure_weight #fea_model.primary_structure_weight
    #wt_wing                    = (1.0+1.0)*wt_primary_structure
    ...
    wt_empty           = (wt_primary_structure + wt_wing + wt_fuselage +  wt_landing_gear + wt_propulsion + output_2.wt_systems + \
    ...
    output.tow               = vehicle.mass_properties.max_takeoff
    ...
    output.primary_structure_weight            = wt_primary_structure
    
    vehicle.mass_properties.payload = output.payload
    vehicle.mass_properties.operating_empty = output.empty
